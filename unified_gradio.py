...
from grape_growth import pest_risk_assessment  
...
def predict_full_pipeline(img, img_type, temp, humidity, soil_ph, nitrogen):
    ...
    pest_advice = pest_risk_assessment(
        pred_class,
        temp,
        humidity,
        soil_ph,
    ...
    advice +
    "\n\n\n"
    "🌱 PLANT GROWTH RECOMMENDATION\n"
    "\n"
    f"{growth_advice}\n\n"
    "\n"
    "🐛 PEST RISK & CONTROL ADVISORY\n"
    "\n"
    f"{pest_advice}"
)
...
with gr.Blocks(css=dark_css) as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            img_type_radio = gr.Radio(["leaf", "fruit", "stem"], label="Image Type", value="leaf")
